using HypeLabs.Connect.Sdk.Generated;
using Microsoft.Kiota.Http.HttpClientLibrary;

namespace HypeLabs.Connect.Sdk;

/// <summary>
/// The entry point to the HypeLabs Connect API. Construct one with your API key and use the fluent request
...
/// touch the generated <see cref="ConnectApiClient"/> plumbing directly.
/// </summary>
public sealed class ConnectClient : ConnectApiClient
{
    /// <summary>
    /// Creates a client authenticated with the given API key, with an internally-managed <see cref="HttpClient"/>.
    /// Prefer <c>AddConnectClient</c> in a DI-based app so the HTTP handler is pooled by the factory.
    /// </summary>
    /// <param name="apiKey">A Connect API key (e.g. <c>hl_live_…</c>).</param>
    public ConnectClient(string apiKey)
    ...
    public ConnectClient(string apiKey, HttpClient httpClient, string? baseUrl = null)
